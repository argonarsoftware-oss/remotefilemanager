...
def handle_self_update(params):
    """Self-update: download pre-built exe from server, swap, and restart."""
    try:
        ...
        # Save new exe next to current one
        new_exe = os.path.join(exe_dir, "RemoteFileManager_new.exe")
        with open(new_exe, "wb") as f:
            f.write(resp.content)

        # Create batch script: wait for process to fully exit, swap, restart, cleanup
        pid = os.getpid()
        old_exe = os.path.join(exe_dir, "RemoteFileManager_old.exe")
        ...
        with open(bat_path, "w") as bat:
            ...
            bat.write(f'del /f "{old_exe}" >nul 2>&1\n')
            bat.write(f'rename "{exe_path}" "RemoteFileManager_old.exe"\n')
            bat.write(f'rename "{new_exe}" "RemoteFileManager.exe"\n')
            bat.write(f'start "" "{exe_path}"\n')
            bat.write(f'timeout /t 5 /nobreak >nul\n')
            bat.write(f'del /f "{old_exe}" >nul 2>&1\n')
            bat.write(f'del "%~f0"\n')
